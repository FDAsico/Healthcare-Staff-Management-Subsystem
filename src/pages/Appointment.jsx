...
const Appointment = () => {
  ...
  // example lang to pero tatanggalin to kasi cconnect to sa dashbaord kung saan andoon yun mga doctors and details
  const availableDoctors = [
    { name: "Dr. Alice Brown", dept: "Cardiology" },
    { name: "Dr. Mark Wilson", dept: "Neurology" },
    { name: "Dr. Sarah Jenkins", dept: "Pediatrics" },
    { name: "Dr. Robert Chen", dept: "Orthopedics" },
    { name: "Dr. Emily Davis", dept: "Dermatology" },
    { name: "Dr. Michael Scott", dept: "General Medicine" },
    { name: "Dr. Lisa Ray", dept: "Oncology" },
    { name: "Dr. James Moore", dept: "Psychiatry" },
    { name: "Dr. Anna Taylor", dept: "Radiology" },
    { name: "Dr. Kevin Lee", dept: "Gastroenterology" },
    { name: "Dr. Sophia White", dept: "Ophthalmology" },
    { name: "Dr. Daniel Green", dept: "Endocrinology" },
  ];
  ...
  useEffect(() => {
    const syncAppointments = () => {
      ...
      const merged = patients.map((p) => {
        const existing = storedAppointments.find((appt) => appt.id === p.id);
        return (
          existing || {
            id: p.id,
            ...
            doctor: "", 
            department: "", 
            date: p.lastVisit || new Date().toISOString().split('T')[0],
            time: "10:00 AM",
  ...
  const handleDoctorAssignment = (id, doctorName) => {
    const doctorObj = availableDoctors.find(d => d.name === doctorName);
    setAppointments((prev) => {
      const updated = prev.map((appt) =>
        appt.id === id 
          ? { ...appt, doctor: doctorName, department: doctorObj ? doctorObj.dept : "" } 
          : appt
      );
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-gray-600">Assign medical staff and confirm visits.</p>
            </div>

            <div className="inline-flex items-center gap-2 rounded-full bg-white px-4 py-2 shadow-sm border border-gray-200">
              <Calendar size={18} className="text-blue-600" />
              <span className="text-sm font-semibold">{appointments.length} Records</span>
            </div>
          </div>
            ...
            {appointments.length === 0 ? (
              ...
              <div className="divide-y divide-gray-100">
                {appointments.map((appt) => (
                  <div key={appt.id} className="flex flex-col lg:flex-row lg:items-center justify-between p-6 hover:bg-gray-50/50 transition-colors gap-6">
                    {/* Patient Info */}
                    <div className="flex gap-4 items-start min-w-[250px]">
                      ...
                      <div>
                        <p className="font-bold text-gray-900 text-lg">{appt.patient}</p>
                    ...
                    <div className="flex flex-1 items-center gap-4">
                      <div className="relative flex-1 max-w-[240px]">
                        ...
                        <select
                          value={appt.doctor}
                          onChange={(e) => handleDoctorAssignment(appt.id, e.target.value)}
                        ...
                        {/* Simple custom arrow to replace default browser UI */}
